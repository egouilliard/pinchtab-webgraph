...
run () {
  ...
  start=$(date +%s)
  out=$(timeout 300 "$DIR/run-recipe.sh" "$@" 2>&1)
  end=$(date +%s)
  states=$(printf '%s\n' "$out" | grep -c '· \[')
...
run "EASY  (add cae @ dashboard)"        --goal "add cae"      --start https://go-staging.leyton.com/dashboard          --out out/bench/easy
# MEDIUM: trigger one tab-click from the start page
run "MEDIUM(add template @ settings)"    --goal "add template" --start https://go-staging.leyton.com/settings           --out out/bench/medium
# HARD: trigger multiple steps away, through a tab, from the dashboard
run "HARD  (add template @ dashboard)"   --goal "add template" --start https://go-staging.leyton.com/dashboard          --out out/bench/hard
echo "=== done ==="
